  ...
  return (
    <div className="champ-match-row">
      <div className="champ-match-team home">
        {match.home?.logo && <Image src={match.home.logo} alt="" width={25} height={25} unoptimized />}
      ...
      <div className="champ-match-team away">
        ...
        {match.away?.logo && <Image src={match.away.logo} alt="" width={25} height={25} unoptimized />}
      </div>
    </div>
  );
}
...
export default async function ChampionshipPage({ params }) {
  ...
  return (
    <div className="page-shell listing-page championship-page">
      ...
      <div className="championship-title-row">
        <div>
          <h1>{config.name}</h1>
          {config.subtitle && <p>{config.subtitle}</p>}
        </div>
        <div className="championship-brand-mark">{config.shortName}</div>
      </div>
        ...
        <>
          ...
          <div className="championship-dashboard">
            <section className="championship-panel championship-table-panel">
              ...
              {result.standings.length ? (
                <div className="championship-table">
                  <div className="champ-table-row head"><span>#</span><span>Équipe</span><span>J</span><span>Diff</span><span>Pts</span></div>
                  {result.standings.map((row) => (
                    <div className="champ-table-row" key={`${row.teamId}-${row.rank}`}>
            ...
            <div className="championship-side-stack">
              ...
              <section className="championship-panel">
                <div className="panel-heading"><h2>Prochains matchs</h2></div>
                {result.upcoming.length ? result.upcoming.slice(0, 5).map((m) => <MatchLine key={`u-${m.id}`} match={m} />) : <div className="champ-empty">Aucun match à venir disponible.</div>}
              </section>
            </div>
          </div>
          ...
          {normalizedSlug === "ligue-1" && result.scorers.length > 0 && (
            <section className="championship-panel championship-scorers">
              ...
              <div className="champ-scorer-grid">
                {result.scorers.slice(0, 5).map((player, index) => (
                  <div className="champ-scorer" key={`${player.playerId}-${index}`}>
                    <b>{index + 1}</b>
