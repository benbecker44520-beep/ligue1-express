...
export async function GET(request) {
  try {
    ...
    const [heartbeat, engine, articles, events] = await Promise.all([
    ...
    return NextResponse.json({
      ok:true,
      autopilot:{status:age != null && age < 300000 ? "healthy" : "attention",autonomous:true,humanReviewRequired:false,heartbeatAt,heartbeatAgeSeconds:age == null ? null : Math.max(0,Math.round(age/1000)),postMatchEngineAt:engine.data?.last_run_at || null},
      last24h:{articlesGenerated:rows.length,articlesPublished:rows.filter(a=>a.status==="published").length,eventsProcessed:eventRows.length,articleNotifications:eventRows.filter(e=>e.event_type==="article_published").length,lineups:eventRows.filter(e=>e.event_type==="lineup").length}
    });
  } catch (error) {
