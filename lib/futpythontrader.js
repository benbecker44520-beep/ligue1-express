...
function buildStandings(rows = []) {
  ...
  const ensure = (name) => {
    ...
    if (!key) return null;
    if (!teams.has(key)) teams.set(key, { teamId: key, team: key, shortName: key, logo: null, played: 0, win: 0, draw: 0, lose: 0, goalsFor: 0, goalsAgainst: 0, diff: 0, points: 0 });
  ...
  rows.forEach((row) => {
    ...
    if (hs > as) { home.win += 1; away.lose += 1; home.points += 3; }
    else if (hs < as) { away.win += 1; home.lose += 1; away.points += 3; }
...
export async function getFutPythonTraderSnapshotUncached(leagueSlug, expectedTeams = 18, season = "2026-2027") {
  ...
  const events = rows.map(mapMatch);
...
export function getFutPythonTraderSnapshot(leagueSlug, expectedTeams = 18, cacheKey = leagueSlug) {
  return unstable_cache(
    ...
    [`ligue1-express-futpythontrader-${cacheKey}-v562`],
    { revalidate: 300 }
  )();
}
