...
// Résultats de secours vérifiés quand le flux principal publie une journée
// partiellement. Ils sont fusionnés (jamais dupliqués) puis deviennent inutiles
// dès que FutPythonTrader fournit les mêmes affiches.
const VERIFIED_RESULT_SUPPLEMENTS = {
  "ligue-2:2026-2027": [
    ["Annecy", "Dijon", 1, 2, "2026-09-18"],
    ["Grenoble", "Clermont", 1, 1, "2026-09-18"],
    ["Laval", "Sochaux", 1, 1, "2026-09-18"],
    ["Pau FC", "Dunkerque", 2, 0, "2026-09-18"],
    ["Reims", "Montpellier", 1, 1, "2026-09-18"],
    ["Rodez", "Nancy", 3, 4, "2026-09-18"],
    ["Boulogne", "Nantes", 1, 2, "2026-09-19"],
    ["Guingamp", "Red Star", 1, 3, "2026-09-19"],
    ["Metz", "St Etienne", 2, 2, "2026-09-19"]
  ].map(([Home, Away, Home_Score, Away_Score, Date], index) => ({
    Match_ID: `verified-l2-2026-j7-${index + 1}`,
    Date,
    Time: index < 6 ? "20:00" : "14:00",
    Round: "7",
    Home,
    Away,
  ...
  "ligue-3:2026-2027": [
    ["Paris 13 Atl.", "Bourg en Bresse", 1, 1],
    ["Concarneau", "Caen", 2, 0],
    ["Rouen", "Aubagne", 1, 0],
    ["Fleury-Merogis", "Thionville", 1, 1],
    ["La Roche", "Valenciennes", 3, 2],
    ["Le Puy-en-Velay", "Villefranche", 4, 1],
    ["Versailles", "Quevilly Rouen", 0, 0],
    ["Amiens", "Orleans", 2, 0]
  ].map(([Home, Away, Home_Score, Away_Score], index) => ({
    Match_ID: `verified-l3-2026-j7-${index + 1}`,
    Date: "2026-09-19",
    Time: "14:45",
    Round: "7",
    Home,
...
function mergeVerifiedResults(rows, leagueSlug, season) {
  const supplements = VERIFIED_RESULT_SUPPLEMENTS[`${leagueSlug}:${season}`] || [];
  if (!supplements.length) return rows;
  const merged = [...rows];
  for (const supplement of supplements) {
    const index = merged.findIndex((row) =>
      normalizeTeamKey(row.Home) === normalizeTeamKey(supplement.Home) &&
      normalizeTeamKey(row.Away) === normalizeTeamKey(supplement.Away)
    );
    if (index === -1) merged.push(supplement);
    else {
      const row = merged[index];
      merged[index] = {
        ...row,
        Round: row.Round || supplement.Round,
        Date: row.Date || supplement.Date,
        Time: row.Time || supplement.Time,
        Home_Score: numberOrNull(row.Home_Score) === null ? supplement.Home_Score : row.Home_Score,
        Away_Score: numberOrNull(row.Away_Score) === null ? supplement.Away_Score : row.Away_Score
      };
    }
  }
  return merged;
...
function mapMatch(row, index, roundOffset = 0, matchesPerRound = 9) {
  ...
  return {
    ...
    timestamp,
    status,
    // Selon la compétition, FutPythonTrader renvoie `Round`, `Matchday`,
    // `Rodada`, une valeur comme "Round 6", ou aucun champ de journée.
    // Dans ce dernier cas, le CSV reste ordonné par journée : on reconstruit
    // donc des blocs complets à partir du nombre d'équipes.
    matchday: roundNumber(row) === null
      ? Math.floor(index / matchesPerRound) + 1
      : roundNumber(row) + roundOffset,
...
export async function getFutPythonTraderSnapshotUncached(leagueSlug, expectedTeams = 18, season = "2026-2027") {
  const rows = mergeVerifiedResults(await fetchDataset(leagueSlug, season), leagueSlug, season);
  ...
  const rawFinishedRounds = rows
    ...
    .map((row) => roundNumber(row))
    .filter((round) => round !== null);
  const maxRawRound = rawFinishedRounds.length ? Math.max(...rawFinishedRounds) : null;
  const minRawRound = rawFinishedRounds.length ? Math.min(...rawFinishedRounds) : null;
  const maxPlayed = standings.length ? Math.max(...standings.map((team) => team.played)) : 0;
  const roundOffset = minRawRound === 0 || (maxRawRound !== null && maxPlayed === maxRawRound + 1) ? 1 : 0;
  const matchesPerRound = Math.max(1, Math.floor(expectedTeams / 2));

  const events = rows.map((row, index) => mapMatch(row, index, roundOffset, matchesPerRound)).map((m) => ({ ...m, home: { ...m.home, logo: badges.get(m.home.name) || null }, away: { ...m.away, logo: badges.get(m.away.name) || null } }));
  ...
  const finishedMatchdays = recent.map((match) => match.matchday).filter(Boolean);
  const currentMatchday = finishedMatchdays.length ? Math.max(...finishedMatchdays) : maxPlayed || null;

  // Conserve le calendrier complet. Certains résultats sont publiés en retard
  // par la source : ces rencontres passées sans score ne doivent pas disparaître
  // de leur page de journée.
  return { standings, recent, upcoming, matches: events, season: season.replace("-", "/"), source: "FutPythonTrader", limited: false, note: null, currentMatchday };
}

export async function getFutPythonTraderSnapshot(leagueSlug, expectedTeams = 18, cacheKey = leagueSlug) {
  const snapshot = await unstable_cache(
    () => getFutPythonTraderSnapshotUncached(leagueSlug, expectedTeams),
    [`ligue1-express-futpythontrader-${cacheKey}-v576`],
    { revalidate: 300 }
  )();
  // Protection contre une entrée de cache vide pendant un redéploiement.
  return snapshot || getFutPythonTraderSnapshotUncached(leagueSlug, expectedTeams);
}
...
export async function getFutPythonTraderTeamSnapshot(leagueSlug, teamName, expectedTeams = 18) {
  ...
  const matches = (snapshot.matches || [...snapshot.recent, ...snapshot.upcoming]).filter((m) =>
...
export async function getFutPythonTraderMatchSnapshot(leagueSlug, matchId, expectedTeams = 18) {
  ...
  const match = (snapshot.matches || [...snapshot.recent, ...snapshot.upcoming]).find((m) => String(m.id) === decoded);
