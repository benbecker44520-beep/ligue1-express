...
function siteUrl() {
  ...
  return "https://foot-francais-express.vercel.app";
}
...
export async function requireAdmin(request) {
  ...
  if (roleError || !allowed) throw new Error("Accès réservé à la rédaction.");
...
export function renderNewsletterHtml({ subject, intro, articles, unsubscribeToken = null }) {
  ...
  const unsubscribe = unsubscribeToken ? `<p style="margin:24px 0 0;font-size:11px;color:#9aa3b2;text-align:center">Tu reçois cet e-mail car tu es inscrit à Foot Français Express. <a href="${base}/api/newsletter/unsubscribe?token=${encodeURIComponent(unsubscribeToken)}" style="color:#6f7785">Se désinscrire</a>.</p>` : "";
...
export async function sendWithResend({ to, subject, html }) {
  ...
  const from = process.env.NEWSLETTER_FROM_EMAIL || "Foot Français Express <onboarding@resend.dev>";
