...
export function buildPostMatchArticle(match) {
  ...
  let scenario;
  if(goals.length){ const sequence=goals.slice(0,6).map(e=>describeGoal(e,match)).join(" "); scenario=`Le scénario du match s'est dessiné au rythme des buts. ${sequence}${goals.length>6?" La fin de rencontre a encore apporté son lot de mouvements au tableau d'affichage.":""}`; }
  else scenario=`Le score est resté bloqué malgré les différentes séquences de la rencontre. Les deux blocs ont résisté et aucune équipe n'est parvenue à convertir ses opportunités en but.`;
  ...
  const conclusion=winner ? `Au coup de sifflet final, ${winner} peut donc savourer ce succès face à ${loser}. Pour les deux équipes, l'attention se tourne désormais vers la prochaine journée${match.leagueName?` de ${match.leagueName}`:""}, avec l'objectif de confirmer ou de rebondir.` : `Ce partage des points laisse forcément des sensations différentes dans les deux camps. ${match.home.name} et ${match.away.name} devront désormais se projeter sur leur prochain rendez-vous et chercher à transformer ce nul en dynamique positive.`;
