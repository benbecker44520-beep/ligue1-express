...
export function buildPostMatchArticle(match) {
  ...
  return { slug:`${slugify(match.home.name)}-${slugify(match.away.name)}-${match.id}`, title, category:"ACTUALITÉS", excerpt, content:[context?`**${context} — Score final : ${scoreLine(match)}.**`:`**Score final : ${scoreLine(match)}.**`,lead,scenario,turningPoint,conclusion].join("\n\n"), image_url:matchVisual(match), related_club_ids:[match.home.id,match.away.id].map(Number).filter(Number.isFinite), status:"published", published_at:new Date().toISOString(), auto_generated:true, source_type:"post_match", source_match_id:String(match.id), review_status:"auto_published", generated_at:new Date().toISOString(), automation_payload:{provider:match.provider,league:match.leagueName,round:match.round,score:match.score,home:match.home,away:match.away,important_events:keyEvents,editorial_version:"v8.32"} };
}
...
function validAutomaticMatch(match) {
  const home=String(match?.home?.name||"").trim(), away=String(match?.away?.name||"").trim();
  const hs=Number(match?.score?.home), as=Number(match?.score?.away);
  return match?.status==="FINISHED" && home && away && home!==away && Number.isFinite(hs) && Number.isFinite(as) && hs>=0 && as>=0;
}

export async function generatePostMatchDraftForMatch(supabase, match) {
  if (!validAutomaticMatch(match)) return { enabled:true, checked:1, created:0, notified:0, skipped:"invalid_match_data" };
...
export async function generatePostMatchDrafts(supabase,{force=false}={}) {
  ...
  const now=Date.now();
  // A football match normally finishes around 2 hours after kick-off. Restrict automatic
  // creation to very recent fixtures so enabling the cron never backfills old matches.
  const fresh=(result.data||[]).filter((match)=>{
    if(!validAutomaticMatch(match)) return false;
    const kickoff=new Date(match.utcDate||0).getTime();
    if(!Number.isFinite(kickoff)||kickoff<=0) return false;
    const age=now-kickoff;
    return age>=75*60*1000 && age<=4*60*60*1000;
  });
  let created=0,notified=0;
  for(const match of fresh){
    const one=await generatePostMatchDraftForMatch(supabase,match);
    created+=Number(one.created||0);
    notified+=Number(one.notified||0);
  }
  return {enabled:true,checked:fresh.length,created,notified};
}
