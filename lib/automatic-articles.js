...
import { getFinishedMatchesForAutomaticArticles } from "@/lib/automatic-match-source";
...
export function buildPostMatchArticle(match) {
  ...
  return { slug:`${slugify(match.home.name)}-${slugify(match.away.name)}-${slugify(match.id)}`, title, category:"ACTUALITÉS", excerpt, content:[context?`**${context} — Score final : ${scoreLine(match)}.**`:`**Score final : ${scoreLine(match)}.**`,lead,scenario,turningPoint,conclusion].join("\n\n"), image_url:matchVisual(match), related_club_ids:[match.home.id,match.away.id].map(Number).filter(Number.isFinite), status:"published", published_at:new Date().toISOString(), auto_generated:true, source_type:"post_match", source_match_id:String(match.id), review_status:"approved", generated_at:new Date().toISOString(), automation_payload:{provider:match.provider,league:match.leagueName,round:match.round,score:match.score,home:match.home,away:match.away,important_events:keyEvents,editorial_version:"v8.32"} };
}

async function notifyAdmins(supabase,article,match){ const {data:admins}=await supabase.from("supporter_profiles").select("user_id").eq("role","admin"); const ids=(admins||[]).map(r=>r.user_id).filter(Boolean); if(!ids.length)return 0; const {data:subs}=await supabase.from("push_subscriptions").select("id,user_id,endpoint,p256dh,auth").in("user_id",ids); let sent=0; for(const s of subs||[]){try{await sendPush(s,{title:"🤖 Article publié automatiquement",body:`${scoreLine(match)} · Le débrief est en ligne.`,icon:"/icon-192.png",badge:"/icon-192.png",type:"editorial_review",url:"/admin?section=articles",tag:`article-auto-${article.id}`});sent++;}catch(e){if([404,410].includes(e?.statusCode))await supabase.from("push_subscriptions").delete().eq("id",s.id);}}return sent; }

async function featureMatchArticle(supabase, match) {
  const {data:article,error}=await supabase
    .from("articles")
    .update({is_featured:true})
    .eq("source_match_id",String(match.id))
    .eq("status","published")
    .select("id")
    .maybeSingle();
...
export async function generatePostMatchDraftForMatch(supabase, match, {feature=true}={}) {
  ...
  if(error?.code==="23505"){
    const featuredArticleId=feature ? await featureMatchArticle(supabase,match) : null;
    return { enabled:true, checked:1, created:0, notified:0, duplicate:true, featuredArticleId };
  }
  if(error) throw error;
  ...
  const featuredArticleId=feature ? await featureMatchArticle(supabase,match) : null;
  return { enabled:true, checked:1, created:1, notified, article, featuredArticleId };
}

export async function generatePostMatchDrafts(supabase,{force=false}={}) {
  if(String(process.env.AUTOMATIC_ARTICLES_ENABLED||"true").toLowerCase()==="false") return {enabled:false,checked:0,created:0,notified:0};
  const {data:previous}=await supabase.from("article_automation_runs").select("last_run_at").eq("run_key",RUN_KEY).maybeSingle();
  const parsedLast=previous?.last_run_at ? new Date(previous.last_run_at).getTime() : 0;
  const last=Number.isFinite(parsedLast) && parsedLast <= Date.now() ? parsedLast : 0;
  if(!force && Date.now()-last<CHECK_INTERVAL_MS) return {throttled:true,checked:0,created:0,notified:0};
  await supabase.from("article_automation_runs").upsert({run_key:RUN_KEY,last_run_at:new Date().toISOString()},{onConflict:"run_key"});
  const result=await getFinishedMatchesForAutomaticArticles({days:1});
  if(!result.ok) throw new Error(result.error||"Matchs terminés indisponibles");
  const now=Date.now();
  // A football match normally finishes around 2 hours after kick-off. Keep a short
  // recovery window so a temporary provider/database failure can be retried the next
  // morning without turning Autopilot into an unrestricted historical backfill.
  const fresh=(result.data||[]).filter((match)=>{
    const kickoff=new Date(match.utcDate||0).getTime();
    if(!Number.isFinite(kickoff)||kickoff<=0) return false;
    const age=now-kickoff;
    return age>=75*60*1000 && age<=36*60*60*1000;
  });
  let created=0,notified=0;
  for(const match of fresh){
    const one=await generatePostMatchDraftForMatch(supabase,match,{feature:false});
    created+=Number(one.created||0);
    notified+=Number(one.notified||0);
  }
  const newest=fresh.reduce((latest,match)=>!latest || Number(match.timestamp||0)>Number(latest.timestamp||0) ? match : latest,null);
  const featuredArticleId=newest ? await featureMatchArticle(supabase,newest) : null;
  return {enabled:true,checked:fresh.length,created,notified,featuredArticleId};
}
