...
import { getEspnChampionshipSnapshot } from "@/lib/espn";
...
export const CHAMPIONSHIPS = {
  ...
  "ligue-2": { slug: "ligue-2", name: "Ligue 2", shortName: "L2", level: "2e échelon", teamCount: 18, provider: "ESPN", espnLeague: "fra.2", tsdbId: "4401" },
  "ligue-3": { slug: "ligue-3", name: "Ligue 3", subtitle: "Championnat professionnel FFF", shortName: "L3", level: "3e échelon", teamCount: 18, provider: "ESPN", espnLeague: "fra.3", tsdbId: "4637" }
};
...
async function getSecondaryChampionship(slug) {
  ...
  try {
    const espn = await getEspnChampionshipSnapshot(config.espnLeague, config.teamCount, slug);
    return {
      ok: true,
      config,
      season: espn.season,
      ...
      recent: espn.recent,
      upcoming: espn.upcoming,
      scorers: [],
      metrics: championshipMetrics(espn.standings),
      source: `ESPN (${config.espnLeague})`,
      limited: false,
      note: null
    };
  } catch (error) {
    try {
      return await getTsdbSnapshotUncached(config, error?.message || "ESPN indisponible");
    } catch (fallbackError) {
      return { ok: false, config, error: fallbackError?.message || error?.message || "Données championnat indisponibles" };
    }
  }
}
