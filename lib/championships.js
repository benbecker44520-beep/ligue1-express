...
export const CHAMPIONSHIPS = {
  ...
  "coupe-de-france": { slug: "coupe-de-france", name: "Coupe de France", subtitle: "La grande coupe du football français", shortName: "CDF", level: "Coupe nationale", provider: "ESPN", type: "cup" }
};
...
async function getTsdbSnapshotUncached(config, fallbackReason = null) {
  ...
  return {
    ...
    limited: !complete,
    note: complete ? null : `Source de secours incomplète : ${standings.length} équipe${standings.length > 1 ? "s" : ""} sur ${config.teamCount}. Foot Français Express préfère masquer un classement tronqué plutôt que d'afficher de fausses statistiques.`,
...
async function getSecondaryChampionship(slug) {
  ...
  try {
    ...
    if (!fpt) throw new Error("Données championnat temporairement indisponibles");
    return {
      ok: true,
      config,
      season: fpt.season,
      ...
      matches: (fpt.matches || [...fpt.recent, ...fpt.upcoming]).sort((a,b) => a.timestamp-b.timestamp),
      limited: false,
      note: null
    };
  } catch (error) {
...
export async function getChampionshipSnapshot(slug) {
  ...
  if (config.type === "cup") {
    const cup = await getEspnCupMatches();
    if (!cup.ok) return { ok: false, config, error: cup.error || "Données Coupe de France indisponibles" };
    const now = Math.floor(Date.now() / 1000);
    const matches = [...cup.data].map((match) => ({
      ...match,
      id: String(match.id).startsWith("cdf:") ? String(match.id) : `cdf:${match.id}`,
      provider: "espn-free",
      league: { slug: "coupe-de-france", name: "Coupe de France", shortName: "CDF", european: false },
      leagueName: "Coupe de France"
    })).sort((a,b) => a.timestamp-b.timestamp);
    const recent = matches.filter((m) => m.status === "FINISHED" || (m.score?.home != null && m.score?.away != null && m.timestamp < now)).sort((a,b) => b.timestamp-a.timestamp);
    const upcoming = matches.filter((m) => m.status !== "FINISHED" && (m.timestamp >= now - 4*3600)).sort((a,b) => a.timestamp-b.timestamp);
    const rounds = [...new Set(matches.map((m) => m.round).filter(Boolean))];
    return { ok: true, config, season: cup.season, standings: [], recent, upcoming, matches, rounds, scorers: [], metrics: null, source: "ESPN", limited: false, note: null, isCup: true };
  }
  ...
  const matchdays = fixtures.data.map((m) => Number(m.matchday)).filter(Boolean);
  const currentMatchday = matchdays.length ? Math.max(...fixtures.data.filter((m) => m.status === "FINISHED").map((m) => Number(m.matchday)).filter(Boolean)) : null;
  return { ok: true, config, season: standings.season || "Actuelle", standings: standings.data, recent: recent.slice(0,200), upcoming: upcoming.slice(0,200), matches: fixtures.data, currentMatchday, scorers: scorers.ok ? scorers.data.slice(0,10) : [], metrics: championshipMetrics(standings.data), source: "football-data.org", limited: false, note: null };
}
