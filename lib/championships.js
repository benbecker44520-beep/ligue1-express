...
import { getFrenchCupMatches } from "@/lib/apifootball";
...
export async function getChampionshipSnapshot(slug) {
  ...
  if (config.type === "cup") {
    const cup = await getFrenchCupMatches();
    ...
    return { ok: true, config, season: cup.season, standings: [], recent, upcoming, matches, rounds, scorers: [], metrics: null, source: "APIfootball", limited: false, note: null, isCup: true };
  }
