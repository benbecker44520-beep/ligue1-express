...
async function loadArticleBySlug(slug) {
  const supabase = createServerSupabaseClient() || createSupabaseClient();
  ...
  if (supabase) {
    ...
    if (error) console.error("Supabase article error:", error.message);
  }
...
// Metadata and page rendering need the same snapshot of the article.
