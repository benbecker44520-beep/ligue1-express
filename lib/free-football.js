...
function recentDates(daysBefore = 1) {
  const safeDays = Math.max(0, Math.min(7, Number(daysBefore) || 0));
  const now = new Date();
  return Array.from({ length: safeDays + 1 }, (_, offset) => {
...
async function recentFinishedUncached(days) {
  // ESPN accepts a single YYYYMMDD date, but rejects the date range that was
  // previously used here. Query each day so finished matches are not hidden.
  const dates = recentDates(days);
  const groups = await Promise.all(
    FREE_COMPETITIONS.flatMap((league) => dates.map((date) => competitionEvents(league, date)))
  );
