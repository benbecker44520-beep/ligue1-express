  ...
  try {
    return await unstable_cache(
      async () => {
        ...
        // Une erreur API ne doit jamais être conservée dans le cache
        if (!result.ok) {
          throw new Error(result.error || "Erreur football-data.org");
        }
      ...
      [`ligue1-footballdata-person-v2-${id}`],
      { revalidate: 21600 }
    )();
  } catch (error) {
    return {
      ok: false,
      error: error?.message || "Fiche joueur temporairement indisponible"
    };
  }
}