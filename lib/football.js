...
async function footballData(path, extraHeaders = {}) {
...
export async function getPlayerSeasonStats(id) {
  if (!id) return { ok: false, error: "Joueur manquant" };
  try {
    return await unstable_cache(
      async () => {
        const result = await footballData(`/persons/${id}/matches?competitions=${COMPETITION}&limit=100`, {
          "X-Unfold-Goals": "true",
          "X-Unfold-Lineups": "true",
          "X-Unfold-Subs": "true"
        });
        if (!result.ok) throw new Error(result.error || "Statistiques joueur indisponibles");
        const a = result.data?.aggregations || {};
        return {
          ok: true,
          data: {
            matchesOnPitch: a.matchesOnPitch ?? null,
            startingXI: a.startingXI ?? null,
      ...
      [`ligue1-footballdata-player-season-v8973-${id}`],
      { revalidate: 900 }
    )();
  } catch (error) {
    return { ok: false, error: error?.message || "Statistiques joueur temporairement indisponibles" };
  }
}
