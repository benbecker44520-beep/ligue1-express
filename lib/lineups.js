...
function isFinishedStatus(value) {
  ...
  return status === "finished" || status === "after et" || status === "after pen.";
}

function playerName(row) {
  return clean(row?.lineup_player || row?.player || row?.player_name || row?.name);
}

function playerNumber(row) {
  return clean(row?.lineup_number || row?.number || row?.shirt_number);
}

function playerPosition(row) {
  return clean(row?.lineup_position || row?.position);
}

function playerId(row) {
  return clean(row?.player_key || row?.player_id || row?.id);
}

function normalizePlayers(rows) {
  if (!Array.isArray(rows)) return [];
  return rows.map((row, index) => ({
    id: playerId(row) || `${index}-${playerName(row)}`,
    name: playerName(row),
    number: playerNumber(row),
...
function normalizeCoach(value) {
  ...
  if (!row) return "";
  return clean(row?.lineup_player || row?.coach || row?.coach_name || row?.name || row);
}

function sideLineup(rawSide = {}) {
  const starters = rawSide?.starting_lineups || rawSide?.startingXI || rawSide?.starting_xi || rawSide?.starters || [];
  const substitutes = rawSide?.substitutes || rawSide?.subs || rawSide?.bench || [];
  return {
    formation: clean(rawSide?.formation || rawSide?.lineup_formation),
    coach: normalizeCoach(rawSide?.coach),
    starters: normalizePlayers(starters),
    substitutes: normalizePlayers(substitutes)
  };
}

export function extractLineups(raw = {}) {
  const block = raw?.lineup || raw?.lineups || raw?.match_lineup || {};
  const home = sideLineup(block?.home || block?.localteam || block?.home_team || {});
...
async function request(params) {
  ...
  try {
    ...
    return { ok:false, error:error?.message || "Compositions indisponibles", data:[] };
  }
}

function matchIdentity(raw) {
  return {
    id: clean(raw?.match_id),
    provider: "apifootball",
    leagueId: clean(raw?.league_id),
    leagueName: clean(raw?.league_name) || "Football français",
    rawStatus: clean(raw?.match_status),
    utcDate: raw?.match_date ? `${raw.match_date}T${clean(raw?.match_time || "12:00").slice(0,5)}:00+02:00` : null,
...
export async function getMatchLineups(matchId) {
  ...
  if (!FRENCH_LEAGUES.has(match.leagueId)) return { ok:false, error:"Compétition non prise en charge" };
...
export async function getFrenchLineupCandidates() {
  ...
  const matches = result.data
    .filter((raw) => FRENCH_LEAGUES.has(clean(raw?.league_id)) && !isFinishedStatus(raw?.match_status))
