...
export async function getApiFootballPlayerProfile(playerName) {
  ...
  const rawSurname = name.split(/\s+/).filter(Boolean).at(-1) || name;
  ...
  if (!result.ok || !result.data?.length) {
    result = await request({ action: "get_players", player_name: rawSurname });
  }
  const expectedInitial = expected.split(" ")[0]?.[0] || "";
  const playerRows = result.ok && Array.isArray(result.data) ? result.data : [];
  const player = playerRows.find((row) => comparablePlayerName(row?.player_name) === expected)
    || playerRows.find((row) => {
  ...
  if (!player) {
    const scorers = await request({ action: "get_topscorers", league_id: "168" });
    const scorerRows = scorers.ok && Array.isArray(scorers.data) ? scorers.data : [];
    const scorer = scorerRows.find((row) => {
      const parts = comparablePlayerName(row?.player_name).split(" ").filter(Boolean);
      return parts.at(-1) === expectedSurname && (!expectedInitial || !parts[0]?.[0] || parts[0][0] === expectedInitial);
    });
    ...
    return {
      ...
      configured: true,
      data: {
        goals: numberOrNull(scorer.goals),
        assists: numberOrNull(scorer.assists),
        yellowCards: null,
        redCards: null,
        appearances: null,
        injured: false,
  ...
  return {
    ...
    data: {
      ...
      assists: numberOrNull(player.player_assists),
